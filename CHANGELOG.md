...
- New `image_extraction` module for AI-based image content extraction
  - `ImageContentExtractor` class for analyzing images and extracting structured data
  - Support for multiple input formats: file paths, raw bytes, and base64-encoded data
  - `ImageType` enum for classifying images (TABLE, CHART_OR_GRAPH, DIAGRAM, PHOTOGRAPH, TEXT_BLOCK, COMPOUND, OTHER)
  - Structured output schemas: `TableData` (markdown), `ChartData` (with data points), `CompoundData` (nested elements)
  - Auto-detection of MIME types from file extensions
- New utility modules:
  - `validators.py` for input validation helpers
  - `io_helpers.py` for I/O utility functions
- Gitleaks setup for secret detection in commits
- Comprehensive test suite for image extraction functionality

### Changed

- Enhanced logging and validation in structure extraction
- Improved AI client error handling and retry logic
